...
export const fig1: State[] = [
  {
    boxes: { f: { x: 320, y: 200, w: 110, h: 150, solid: 0, firm: 1, label: 'f' } },
    bounds: { F: { x: 265, y: 125, w: 110, h: 150, o: 0 } },
    wires: {
      X: { from: { x: 10, y: 200 }, to: { b: 'f', side: 'in' }, label: 'X', note: 'inputs', lt: 0.3 },
      Y: { from: { b: 'f', side: 'out' }, to: { x: 630, y: 200 }, label: 'Y', note: 'outputs', lt: 0.7 },
    },
  ...
  },
  {
    ...
    wires: {
      X: { from: { x: 10, y: 200 }, to: { b: 'f', side: 'in' }, label: 'X', note: 'inputs', lt: 0.5 },
      Y: { from: { b: 'f', side: 'out' }, to: { x: 630, y: 200 }, label: 'Y', note: 'outputs', lt: 0.5 },
...
const T1 = { t: `<i>f</i> : <i>X</i> ${TO} <i>Y</i>`, g: 'in: inputs · out: outputs' };
...
const f2 = (inside: boolean): State => ({
  boxes: {
    ...
    g: { x: 250, y: 250, w: 64, h: 60, solid: 0, label: 'g', blue: inside ? 1 : 0 },
  },
  bounds: { F: inside ? { x: 178, y: 92, w: 348, h: 216, blue: 1 } : { x: 386, y: 92, w: 140, h: 216, blue: 1 } },
  wires: {
    A: { from: { x: 30, y: 160 }, to: { b: 'f', side: 'in', dy: -40 }, label: 'A', note: 'materials', lt: 0.14 },
    O: { from: { x: 30, y: 250 }, to: { b: 'g', side: 'in' }, label: 'O', note: 'objective', lt: 0.4 },
    P: { from: { b: 'g', side: 'out' }, to: { b: 'f', side: 'in', dy: 50 }, label: 'P', note: 'plan', lt: 0.5, blue: inside ? 1 : 0 },
    Y: { from: { b: 'f', side: 'out' }, to: { x: 610, y: 200 }, label: 'Y', note: 'product', lt: 0.65 },
  },
});
export const fig2: State[] = [f2(false), f2(true), f2(true)];
const T2b = { t: `<i>f</i> ${CO} (1<sub><i>A</i></sub> ${OT} <i>g</i>) : <i>A</i> ${OT} <i>O</i> ${TO} <i>Y</i>`, g: 'in: materials, objective · out: product' };
export const fig2Types: TypeLine[] = [
  { t: `<i>f</i> : <i>A</i> ${OT} <i>P</i> ${TO} <i>Y</i>`, g: 'in: materials, plan · out: product' },
  T2b, T2b,
];

// ── Figure 3 — replacement, isotopy, surgery ────────────────────────────────────────────
// viewBox 740 × 460. The boundary runs from x = 100 to x = 640; wire words sit outside it, centred in the margins.
...
const L = 56, Rm = 690;
const Ow = (x1: number) => ({ label: 'O', note: 'objective', lt: at(12, x1, L) });
const Aw = { label: 'A', note: 'materials', lt: at(12, 256, L) };
const Yw = (x0: number) => ({ label: 'Y', note: 'product', lt: at(x0, 736, Rm) });
...
const f3a = (e2solid: number): State => ({
  ...
  wires: {
    O: { from: { x: 12, y: 140 }, to: { b: 'c', side: 'in' }, ...Ow(136) },
    A: { from: { x: 12, y: 318 }, to: { b: 'e1', side: 'in', dy: 18 }, ...Aw },
    ...
    Y: { from: { b: 'e3', side: 'out' }, to: { x: 736, y: 300 }, ...Yw(564) },
  },
  labels: plans(108),
});
const f3c: State = {
  ...
  wires: {
    O: { from: { x: 12, y: 140 }, to: { b: 'c', side: 'in' }, ...Ow(156) },
    A: { from: { x: 12, y: 318 }, to: { b: 'e1', side: 'in', dy: 18 }, ...Aw },
    p1: { from: { b: 'c', side: 'out', dy: 20 }, to: { b: 'e1', side: 'in', dy: -22 }, bend: 0.5 },
    p2: { from: { b: 'c', side: 'out', dy: 0 }, to: { b: 'e2', side: 'in', dy: -22 }, bend: 0.8 },
    p3: { from: { b: 'c', side: 'out', dy: -20 }, to: { b: 'e3', side: 'in', dy: -22 }, bend: 0.9 },
    m1: { from: { b: 'e1', side: 'out' }, to: { b: 'e2', side: 'in', dy: 18 } },
    m2: { from: { b: 'e2', side: 'out' }, to: { b: 'e3', side: 'in', dy: 18 } },
    Y: { from: { b: 'e3', side: 'out' }, to: { x: 736, y: 300 }, ...Yw(588) },
  },
  labels: plans(174),
};
const f3d: State = {
  ...
  wires: {
    O: { from: { x: 12, y: 150 }, to: { b: 'k', side: 'in' }, ...Ow(164) },
    A: { from: { x: 12, y: 318 }, to: { b: 'e1', side: 'in', dy: 18 }, label: 'A', note: 'materials', lt: at(12, 262, L) },
    ...
    Z: { from: { b: 'h', side: 'out' }, to: { x: 736, y: 196 }, label: 'Z', note: 'new product', lt: at(618, 736, Rm), blue: 1 },
    Y: { from: { b: 'e3', side: 'out' }, to: { x: 736, y: 300 }, ...Yw(558) },
  },
};
export const fig3: State[] = [f3a(0), f3a(1), f3c, f3d];
const T3 = { t: `<i>F</i> : <i>A</i> ${OT} <i>O</i> ${TO} <i>Y</i>`, g: 'in: materials, objective · out: product' };
export const fig3Types: TypeLine[] = [T3, T3, T3, { t: `<i>F</i>′ : <i>A</i> ${OT} <i>O</i> ${TO} <i>Y</i> ${OT} <i>Z</i>`, g: 'in: materials, objective · out: product, new product' }];
