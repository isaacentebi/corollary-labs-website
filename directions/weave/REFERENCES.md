...
Company structure (IA-BRIEF). Home: hero (the real <h1> is woven into the cloth as damask; headline, subline, Contact us / Our approach in plain text; the first saffron thread draws down beside the text, and the re-weave never runs under the copy; visitors can click to insert threads) → Capabilities (one cloth strip, four threads, one column per capability) → Approach (intro, then a pinned story of one screen of scroll: 01 The organisation, 02 An agent enters (close-up, live draft), 03 It reorganises (draft rewrites), 04 The change spreads (pull-back); the legend band is also its navigation; three principles) → Research (latest three essays as threads, All research →) → Company (About line, four team placeholders, About the company →) → Contact ([EMAIL] as plain text). Main nav: Capabilities · Approach · Company · Contact. Footer: Capabilities · Approach · Research · About · Team · Contact, email, ©. Essays are labelled Research everywhere and stay at /essays/.
